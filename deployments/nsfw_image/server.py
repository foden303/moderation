...
import proto.nsfw_image_pb2 as nsfw_image_pb2
import proto.nsfw_image_pb2_grpc as nsfw_image_pb2_grpc
...
logger = logging.getLogger("nsfw_image")
...
# gRPC Servicer
class NSFWImageDetectorServicer(nsfw_image_pb2_grpc.NSFWImageDetectorServicer):
    ...
    async def Predict(self, request, context):
        try:
            ...
            return nsfw_image_pb2.PredictResponse()
    ...
    async def PredictBatchFromURLs(self, request, context):
        ...
        for url in request.urls:
            batch_result = nsfw_image_pb2.BatchPredictionResult(url=url)
        ...
        return nsfw_image_pb2.PredictBatchResponse(predictions=predictions)
    ...
    async def HealthCheck(self, request, context):
        ...
        try:
            ...
            return nsfw_image_pb2.HealthCheckResponse(
                status="healthy",
                model=MODEL_NAME,
                device="ray-serve",
            )
            ...
            context.set_details(str(e))
            return nsfw_image_pb2.HealthCheckResponse(status="unhealthy")
...
async def run_grpc_server(model_handle):
    ...
    nsfw_image_pb2_grpc.add_NSFWImageDetectorServicer_to_server(
        NSFWImageDetectorServicer(model_handle),
        server
    )
...
def main():
    ...
    app = NSFWImageModel.bind()
    handle = serve.run(app, name="nsfw_image_model", route_prefix="/nsfw-image")
